...
int parse_prm(char const *fname, FHMD *fh)
{
    ...
    while(fscanf(fprm, "%s", line) != -1)
    {
        ...
        else if(!strcmp(line, "NF") || !strcmp(line, "nf"))
            ok = assign_double_value(&fh->NF, line, fprm);
        else if(!strcmp(line, "Uflow_x") || !strcmp(line, "uflow_x"))
            ok = assign_double_value(&fh->Uflow_x, line, fprm);
        ...
        else if(!strcmp(line, "eps_hy_rho"))
            ok = assign_double_value(&fh->eps_hy_rho, line, fprm);
        else if(!strcmp(line, "eps_hy_mom"))
            ok = assign_double_value(&fh->eps_hy_mom, line, fprm);
        else if(!strcmp(line, "eps_av_rho"))
            ok = assign_double_value(&fh->eps_av_rho, line, fprm);
        else if(!strcmp(line, "eps_av_mom"))
            ok = assign_double_value(&fh->eps_av_mom, line, fprm);
        ...
        else if(!strcmp(line, "Flow_output"))
            ok = assign_int_value(&fh->step_flow_output, line, fprm);
