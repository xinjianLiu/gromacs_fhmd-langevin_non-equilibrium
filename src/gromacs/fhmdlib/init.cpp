...
int fhmd_init(matrix box, int N_atoms, real mass[], rvec x[], double dt_md, gmx_mtop_t *mtop, t_commrec *cr, FHMD *fh)
{
    ...
    if(MASTER(cr))
    {
        ...
        fh->eps_hy_rho  = 0;
        fh->eps_hy_mom  = 0;
        fh->eps_av_rho  = 0.66666666;
        fh->eps_av_mom  = 0.66666666;
        fh->S_berendsen = 1;
        fh->tau         = 0.1;
        fh->N[0]        = 9;
        fh->N[1]        = 9;
        ...
        fprintf(fw, "NF = %g                  ; Non-equilibrium flow selection (0- Const velocity: Uflow_x=0, 1-poiseuille, 2-couette)\n", fh->NF);
        fprintf(fw, "Uflow_x = %g             ; maximum velocity in the x-direction for Non-equilibrium flow \n\n", fh->Uflow_x);
        ...
        if(fh->NF == 0)
        {
        	printf(MAKE_GREEN "FHMD: NF = %g,  Constant flow \n", fh->NF);
            fh->NF_type = constantflow;
        }
        else if(fh->NF==1)
        {
        	printf(MAKE_GREEN "FHMD: NF = %g,  Poiseuille flow \n", fh->NF);
            fh->NF_type = poiseuille;
        }
        else
		{
        	printf(MAKE_GREEN "FHMD: NF = %g,  Couette flow \n", fh->NF);
        	fh->NF_type = couette;
		}

        printf(MAKE_GREEN "FHMD: Uflow_x = %g, maxium velocity in the x-direction for Non-equilibrium flow \n", fh->Uflow_x);
        ...
        if(fh->scheme == Two_Way)
        {
            ...
            printf("FHMD: epsilon-hybrid adjustment: eps_hy_rho = %g [--], eps_hy_mom = %g [--]\n", fh->eps_hy_rho, fh->eps_hy_mom);
            printf("FHMD: Artificial_viscocity_dissipator: eps_av_rho = %g [--], eps_av_mom = %g [--]\n", fh->eps_av_rho, fh->eps_av_mom);
            ...
            fprintf(fw, "eps_hy_rho = %g         ; Eps_hy_rho parameter (epsilon-hybrid scheme for rho' )\n", fh->eps_hy_rho);
            fprintf(fw, "eps_hy_mom = %g         ; Eps_hy_rho parameter (epsilon-hybrid scheme for mom' )\n", fh->eps_hy_mom);
            fprintf(fw, "eps_av_rho = %g         ; Eps_av_rho parameter (Artificial viscocity term dissipator for rho* )\n", fh->eps_av_rho);
            fprintf(fw, "eps_av_mom = %g         ; Eps_av_rho parameter (Artificial viscocity term dissipator for mom* )\n\n", fh->eps_av_mom);
        ...
        printf("FHMD: Output flow info after %d MD time steps\n", fh->step_flow_output);
        fprintf(fw, "Flow_output  = %d           ;  Step output flow info\n", fh->step_flow_output);
    ...
    fh->mpi_linear = (double*)malloc(27*fh->Ntot*sizeof(double));   // 27 components
    ...
    for(int i = 0; i < FHMD_FLOW_LAYERS; i++)
    {
        fh->avg_vel_tot_n[i]   = 0;
        fh->avg_vel_S_tot_n[i] = 0;
        fh->avg_n_tot_n[i]     = 0;
    }
    ...
    fh->ufh_flow = (double*)calloc(fh->N[1], sizeof(double));
    fh->ufh_flow_tot = (double*)calloc(fh->N[1], sizeof(double));
    fh->avg_ufh_flow_tot= (double*)calloc(fh->N[1], sizeof(double));

    if(fh->ufh_flow == NULL ||fh->ufh_flow_tot == NULL || fh->avg_ufh_flow_tot == NULL)
    {
        if(MASTER(cr)) printf(MAKE_RED "\nFHMD: ERROR: Out of memory (Statistics allocator)\n" RESET_COLOR "\n");
        fflush(stdout);
        exit(3);
