                       ...
                       gmx_bool bNH, gmx_bool bPR, t_commrec *cr, FHMD *fh)
{
    ...
    int          nbr[8];
    dvec         xi;
    dvec         f_fh, u_fh, u_fh_flow, alpha_term, beta_term, grad_ro;// A new scheme for non-equilibrium flow
    ...
    else
    {
        ...
        for (n = start; n < nrend; n++)
        {
            if ((ptype[n] != eptVSite) && (ptype[n] != eptShell))
            {
                ...
                trilinear_interpolation(u_fh_flow,    xi, INTERPOLATE(u_fh_flow));// A new scheme for non-equilibrium flow
                ...
                for (d = 0; d < DIM; d++)
                {
                    ...
                    else if(fh->scheme == Two_Way)
                    {
                       ...
                       xprime[n][d] = x[n][d] + ((1 - S)*vn + S*u_fh_flow[d] + S*(1 - S)*grad_ro[d]*arr[ind].inv_ro)*(exp(gamma_lang*dt) - 1)/gamma_lang;
                   
                        if (fh->NF_type == couette && d == YY)
                        {
                        	if(xprime[n][d] < 0)
                        		xprime[n][0] += U_flow[d]*dt;
