...
void Uflow_init(FHMD *fh, t_commrec *cr)
{
    ...
    for(int k = 0; k < NZ; k++)
    {
    	for(int j = 0; j < NY; j++)
    	{
    		for(int i = 0; i < NX; i++)
            {
                ...
                for(int d = 0; d < DIM; d++)
                {
                    if(fh->NF_type == constantflow)
                    {
                    	arr[C].Uflow[d] = U_flow[d];
                    }
                    else if(fh->NF_type == poiseuille)
                    {
                    	arr[C].Uflow[d]= U_flow[d]*1/3*fh->N_md[1]*fh->N_md[1]/9/9-U_flow[d]*((fh->grid.c[C][YY]-fh->box05[1])*(fh->grid.c[C][YY]-fh->box05[1])/(fh->box[1]/fh->N_md[1]*9/2)/(fh->box[1]/fh->N_md[1]*9/2));
                    	//arr[C].Uflow[d]= U_flow[d]*1/3*fh->N_md[1]*fh->N_md[1]/fh->N[1]/fh->N[1]-U_flow[d]*((fh->grid.c[C][YY]-fh->box05[1])*(fh->grid.c[C][YY]-fh->box05[1])/(fh->box[1]/fh->N_md[1]*fh->N[1]/2)/(fh->box[1]/fh->N_md[1]*fh->N[1]/2));
                    	//arr[C].Uflow[d]=  U_flow[d]*(1-(fh->grid.c[C][YY]-fh->box05[1])*(fh->grid.c[C][YY]-fh->box05[1])/(fh->box[1]/fh->N_md[1]*fh->N[1]/2)/(fh->box[1]/fh->N_md[1]*fh->N[1]/2));
                    }
                    else
                    {
                    	arr[C].Uflow[d] = U_flow[d]*(fh->grid.c[C][YY]/fh->box[YY] - 0.5);
                    }
                }
            }
        }
            ...
            for(int j = 0; j < NY; j++)
            {
                for(int i = 0; i < NX; i++)
                {
                    ...
                    for(int d = 0; d < DIM; d++)
                    {
                        ...
                        arr[C].m_star[d]  = arr[C].uro_md_flow[d]; 							// A new scheme for non-equilibrium flow
                        arr[C].mn_star[d] = arr[C].uro_md_flow[d]; 							// A new scheme for non-equilibrium flow
                        arr[C].u_fh[d]    = arr[C].uro_md_flow[d]/arr[C].ro_md_prime; 		// A new scheme for non-equilibrium flow
                        arr[C].u_fh_flow[d] = arr[C].u_fh[d]+arr[C].Uflow[d];				// A new scheme for non-equilibrium flow
                        arr[C].u_fh_n[d]  = arr[C].u_fh[d];

                        for(int d1 = 0; d1 < DIM; d1++)
                        {
                            //arr[L].uf[d1][d]  = 0.5*(arr[CL].uro_md[d1]/arr[CL].ro_md_prime + arr[C].uro_md[d1]/arr[C].ro_md_prime);
                            arr[L].uf[d1][d]  = 0.5*(arr[CL].uro_md_flow[d1]/arr[CL].ro_md_prime + arr[C].uro_md_flow[d1]/arr[C].ro_md_prime);// A new scheme for non-equilibrium flow
...
void FH_predictor(FHMD *fh)
{
    ...
    ivec   ind;
    double DT = fh->dt_FH;
    dvec   FP, FS, PG, AVT_RHO, AVT_MOM, TAU, TAURAN;
    matrix TAUL, TAUR;
    double BP, BS, QP, QS;
    double gamma_lang;
        ...
        for(int j = 0; j < NY; j++)
        {
            for(int i = 0; i < NX; i++)
            {
                ...
                for(int d = 0; d < DIM; d++)
                {
                    ...
                    AVT_RHO[d] = a[CR].ro_star-2*a[C].ro_star+a[CL].ro_star;
                }

                //QP = -fh->alpha*SC*(1 - SC)*(a[C].ro_fh - a[C].ro_md);
                QP = -fh->eps_hy_rho*fh->alpha*SC*(1 - SC)*a[C].ro_prime -(1-fh->eps_hy_rho)*fh->alpha*SC*(1 - SC)*(a[C].ro_fh - a[C].ro_md);
                QS = fh->alpha*SC*(1 - SC)*a[C].ro_prime;
                ...
                a[C].AVT_RHO_P = 0.5*fh->eps_av_rho*0.5*(1-SC)*SUM(AVT_RHO);

                if(fh->grid.md[C] == FH_zone) a[C].MDS_RHO_N = 0;

                a[C].ron_star  = a[C].ro_star  + 0.5*DT*(-SUM(FS) + QS +a[C].MDS_RHO_N)+a[C].AVT_RHO_P;
                ...
                for(int dim = 0; dim < DIM; dim++)
                {
                    ...
                    for(int d = 0; d < DIM; d++)
                    {
                        ...
                        AVT_MOM[d] = a[CR].m_star[dim]-2*a[C].m_star[dim]+a[CL].m_star[dim];
                    }

                    // Beta-terms
                    //BP = -fh->beta*SC*(1 - SC)*(a[C].ro_fh*a[C].u_fh[dim] - a[C].uro_md[dim]);
                    BP = -fh->eps_hy_mom*fh->beta*SC*(1 - SC)*a[C].m_prime[dim]-(1-fh->eps_hy_mom)*fh->beta*SC*(1 - SC)*(a[C].ro_fh*a[C].u_fh[dim] - a[C].uro_md[dim]);
                    ...
                    if(fh->grid.md[C] == FH_zone) a[C].MDS_MOM_N[dim] = 0;

                    a[C].AVT_MOM_P[dim] = 0.5*fh->eps_av_mom*0.5*(1-SC)*SUM(AVT_MOM);

                    gamma_lang = arr[C].gamma_cell;

                    a[C].mn_star[dim]  = a[C].m_star[dim]  + 0.5*DT*(-SUM(FS) + SC*a[C].f_fh[dim] + BS + a[C].MDS_MOM_N[dim]- gamma_lang*a[C].uro_md[dim])+a[C].AVT_MOM_P[dim];
...
void FH_corrector(FHMD *fh)
{
    ...
    ivec   ind;
    double DT = fh->dt_FH;
    dvec   FP, AP, FS, PG, AVT_RHO, AVT_MOM, TAU, TAURAN; // A means artificial
    matrix TAUL, TAUR;
    double BP, BS, QP, QS, MDS, QPI, BPI; //I means implicit
    double gamma_lang;
        ...
        for(int j = 0; j < NY; j++)
        {
            for(int i = 0; i < NX; i++)
            {
                ...
                for(int d = 0; d < DIM; d++)
                {
                    ...
                    AP[d] += fh->eps_rho*(a[CR].ro_prime - 2.0*a[C].ro_prime + a[CL].ro_prime)/DT;

                    if(fh->grid.md[C] == boundary) AP[d] = 0;
                }

                QPI = 1/(1+0.5*DT*fh->alpha*SC*(1 - SC)*fh->eps_hy_rho);

                QP = -fh->alpha*SC*(1 - SC)*(a[C].ro_fh - a[C].ro_md_prime);

                // Mass conservation
                //a[C].ronn_prime = a[C].ron_prime + 0.5*DT*(-SUM(FP) + SUM(QP) + QPE);
                a[C].ronn_prime = (a[C].ron_prime + 0.5*DT*(-SUM(FP) + SUM(AP)+ (1-fh->eps_hy_rho)*QP))*QPI; // implicit QP
                ...
                for(int dim = 0; dim < DIM; dim++)
                {
                    for(int d = 0; d < DIM; d++)
                    {
                        ...
                        AP[d] = fh->eps_mom*(a[CR].m_prime[dim] - 2.0*a[C].m_prime[dim] + a[CL].m_prime[dim])/DT;

                        if(fh->grid.md[C] == boundary) AP[d] = 0;
                    }

                    // Beta-term for m_prime
                    BPI = 1/(1+0.5*DT*fh->beta*SC*(1 - SC)*fh->eps_hy_mom);
                    ...
                    a[C].mnn_prime[dim] = (a[C].mn_prime[dim] + 0.5*DT*(-SUM(FP) + SUM(AP)+ (1-fh->eps_hy_mom)*BP))*BPI; // implicit BP
        ...
        for(int j = 0; j < NY; j++)
        {
            for(int i = 0; i < NX; i++)
            {
                ...
                for(int d = 0; d < DIM; d++)
                {
                    ...
                    AVT_RHO[d] = a[CR].ron_star-2*a[C].ron_star+a[CL].ron_star;
                }

                QS = fh->alpha*SC*(1 - SC)*a[C].ronn_prime;
                a[C].AVT_RHO_C = 0.5*fh->eps_av_rho*0.5*(1-SC)*SUM(AVT_RHO);

                // Mass conservation
                a[C].ro_star  = a[C].ron_star  + 0.5*DT*(-SUM(FS) + QS +a[C].MDS_RHO_N) +a[C].AVT_RHO_C;
                ...
                for(int dim = 0; dim < DIM; dim++)
                {
                    ...
                    for(int d = 0; d < DIM; d++)
                    {
                        ...
                        AVT_MOM[d] = a[CR].mn_star[dim]-2*a[C].mn_star[dim]+a[CL].mn_star[dim];
                    }
                    ...
                    a[C].AVT_MOM_C[dim] = 0.5*fh->eps_av_mom*0.5*(1-SC)*SUM(AVT_MOM);

                    gamma_lang = fh-> arr[C].gamma_cell;

                    // Momentum conservation
                    a[C].m_star[dim] = a[C].mn_star[dim] + 0.5*DT*(-SUM(FS) + SC*a[C].f_fh[dim] + BS+a[C].MDS_MOM_N[dim]- gamma_lang*a[C].uro_md[dim])+a[C].AVT_MOM_C[dim];
                    a[C].u_fh[dim]   = (a[C].m_star[dim] + a[C].mnn_prime[dim])/a[C].ro_fh;
                    a[C].u_fh_flow[dim]   = a[C].u_fh[dim]+arr[C].Uflow[dim];// A new scheme for non-equilibrium flow
                }
