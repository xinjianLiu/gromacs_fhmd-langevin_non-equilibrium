    ...
    while (!bLastStep)
    {
        ...
        if (!bRerunMD || rerun_fr.bV || bForceUpdate)
        {
            ...
            else /******************** FHMD coupling ********************/
            {
                ...
                /*
                 * FHMD: Initialize the flow function
                 */
                if(fhmd.step_MD == 0)
                    Uflow_init(&fhmd, cr);
...
//#endif
                ...
                /*
                 * FHMD: Calculate average velocity for flow
                 */
                fhmd_flow_n_avg(&fhmd, state->x, state->v,mdatoms->homenr, cr);
                fhmd_flow_m_avg(&fhmd, state->x, state->v,mdatoms->massT,mdatoms->homenr, cr);
                fhmd_flow_ufh_avg(&fhmd, cr);

                if(MASTER(cr) && (fhmd.Noutput > 0))
                    if(fhmd.step_MD > fhmd.step_flow_output && !(fhmd.step_MD % fhmd.Noutput))
                    {
                    	fhmd_flow_n_avg_write(&fhmd);
                    	fhmd_flow_m_avg_write(&fhmd);
                    	fhmd_flow_ufh_avg_write(&fhmd);
                    }
